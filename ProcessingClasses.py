...
# NSaber, 2021-Dec-11, Added Functionality
#------------------------------------------#
...
class DataProcessor:
    ...
    def select_cd(table: list, cd_idx: int) -> DC.CD:
        ...
        try:
            ...
            print('ID should be an integer', e, e.__doc__)
        for row in table:
            if row.cd_id == cd_idx:
                return row
        raise Exception('CD/Album index not found')
    ...
    def add_track(track_info: tuple, cd: DC.CD) -> None:
        ...
        # TODone add code as required
        trk_pos, trk_title, trk_len = track_info
        try:
            trk_pos = int(trk_pos)
        except:
            raise Exception('Position should be an integer')
        track = DC.Track(trk_pos, trk_title, trk_len)
        cd.add_track(track)
